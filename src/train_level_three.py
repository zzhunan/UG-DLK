...
load_path_one = f'{settings.path}/training'+'/level_one_un_4_1_con/'
load_path_two = f'{settings.path}/training'+'/level_two_un_4_1_con/'
save_path = f'{settings.path}/training'+'/level_three_un_4_1_con/'
...
# corr_uncertainty.load_weights(load_path_two + 'epoch_' + str(10) + "un") # map and earth
corr_uncertainty.load_weights(load_path_two + 'epoch_' + str(settings.train_stage2) + "un") # coco
...
for current_epoch in range(settings.train_stage3):
    ...
    for iters in range(100000000):
        ...
        with tf.GradientTape() as tape:
            ...
            corr_attention = local_cost_volume(template_feature_map_three, input_warped_to_template_three, 4)
            log_var = corr_uncertainty.call(corr_attention)
            B, H, W, C = input_warped_to_template_three.shape
            up_log_var = tf.image.resize(log_var, size=(H, W), method=tf.image.ResizeMethod.BILINEAR)
            ssim_middle = ssim_middle_three + tf.reduce_mean(tf.multiply(tf.exp(-up_log_var),compute_ssim(template_feature_map_three,input_warped_to_template_three)) + up_log_var)  #
        ...
        all_parameters = level_three_template.trainable_variables+level_three_input.trainable_variables+ corr_uncertainty.trainable_variables
    ...
    dfhistory.to_csv(f'{settings.path}/training/dfhistory_level_un3_4_1.csv', index=False)

...
class data_loader_MSCOCO():
  ...
  def data_read_batch(self,batch_size=8):
    ...
    for i in range(batch_size):
      ...
      # img_name = 'COCO_val2014_000000540180.jpg' #红绿灯
      img_name = 'COCO_val2014_000000375363.jpg' # 树枝
      # img_name = 'COCO_val2014_000000065008.jpg'  # 草坪
      # img_name = 'COCO_val2014_000000375363.jpg'
      # img_name = 'COCO_val2014_000000375363.jpg'
      ...
      input_path = self.input_path+img_name #自己加的读取可视化照片
      template_path = self.template_path+img_name #自己加的读取可视化照片
    ...
    return np.asarray(input_all).astype(np.float32),np.asarray(label_u).astype(np.float32),np.asarray(label_v).astype(np.float32),np.asarray(template_all).astype(np.float32),input_path,template_path
...
class data_loader_DPDN():

    def __init__(self, dataset_name='train'):
        if dataset_name == 'train':
            self.img_path = glob.glob('../Dataset/DPDN/train2014_input/*')
            self.input_path = '../Dataset/DPDN/train2014_input/'
            self.label_path = '../Dataset/DPDN/train2014_label/'
            self.template_path = '../Dataset/DPDN/train2014_template/'

        elif dataset_name == 'val':
            self.img_path = glob.glob('../Dataset/DPDN/val2014_input/*')
            self.input_path = '../Dataset/DPDN/val2014_input/'
            self.label_path = '../Dataset/DPDN/val2014_label/'
            self.template_path = '../Dataset/DPDN/val2014_template/'
    ...
    def data_read_batch(self, batch_size=8):
        ...
        for i in range(batch_size):
            ...
            part1, part2 = "Depth", ".bmp"
            template_img_name = f"{part1}_{index}{part2}"
            ...
            input_img = plt.imread(self.input_path + img_name) / 255.0
            template_img = plt.imread(self.template_path + template_img_name) / 255.0

            input_path = self.input_path + img_name  # 自己加的读取可视化照片
            template_path = self.template_path + template_img_name  # 自己加的读取可视化照片



            # with open(self.label_path + img_name[:(len(img_name) - 4)] + '_label.txt', 'r') as outfile:

            with open(self.label_path + str(index) + '_label.txt', 'r') as outfile:
                data = json.load(outfile)
            u_list = [data['location'][0]['top_left_u'], data['location'][1]['top_right_u'],
                      data['location'][3]['bottom_right_u'], data['location'][2]['bottom_left_u']]
            v_list = [data['location'][0]['top_left_v'], data['location'][1]['top_right_v'],
                      data['location'][3]['bottom_right_v'], data['location'][2]['bottom_left_v']]

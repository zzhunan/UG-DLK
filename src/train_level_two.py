...
load_path = f'{settings.path}/training'+'/level_one_un_4_1_con/'
save_path = f'{settings.path}/training'+'/level_two_un_4_1_con/'
...
# corr_uncertainty.save_weights(save_path + 'epoch_' + str(10) + "un")
# corr_uncertainty.load_weights(load_path + 'epoch_' + str(10) + "un") # map and earth
corr_uncertainty.load_weights(load_path + 'epoch_' + str(settings.train_stage1) + "un")  #coco
...
for current_epoch in range(settings.train_stage2):
    ...
    for iters in range(100000000):
        ...
        if len(np.shape(input_img))<2:
          ...
          corr_uncertainty.save_weights(save_path + 'epoch_' + str(1 + current_epoch) + "un")  # 自己加的
          break
        ...
        with tf.GradientTape() as tape:
            ...
            corr_attention = local_cost_volume(template_feature_map_two, input_warped_to_template_two, 4)
            log_var = corr_uncertainty.call(corr_attention)
            B, H, W, C = input_warped_to_template_two.shape
            up_log_var = tf.image.resize(log_var, size=(H, W), method=tf.image.ResizeMethod.BILINEAR)
            ssim_middle = ssim_middle_two + tf.reduce_mean(tf.multiply(tf.exp(-up_log_var),compute_ssim(template_feature_map_two,input_warped_to_template_two)) + up_log_var)  #
        ...
        all_parameters=level_two_template.trainable_variables + level_two_input.trainable_variables + corr_uncertainty.trainable_variables
    ...
    dfhistory.to_csv(f'{settings.path}/training/dfhistory_level_un2_4_1.csv', index=False)
